...
public class MainClass {
	...
	public static String MainExecute() throws FileNotFoundException, IOException, SQLException {
		...
		for (i = 0; i < rulecount; ++i) //i is the rule counter
			dummy = ExecuteRule(i, myDB); //Will execute each rule by sending the Rule counter id
		
		return ("Completed processing " + i + " rules");
	}

	private static int ExecuteRule(int rulecounter, DBExecute myDB) {
		...
		for (int i = 1; i<= RuleTypeCount; ++i){	
			myDB.ExecRule_RuleTypeNum(RuleID, i);	//i represents the Right Rule Type Number
		}
